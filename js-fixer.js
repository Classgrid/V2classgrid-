...
for (const f of files) {
  ...
  for (const regex of tests) {
    content = content.replace(regex, (match, prefix) => {
      // The prefix will contain 'border-radius: '
      return match.substring(0, match.length - match.match(/50%/)[0].length) + '8px';
    });
  }
  ...
  for (const regex of inlineStyles) {
    content = content.replace(regex, (match, prefix, group2, group3) => {
