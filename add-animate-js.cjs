...
const filePath = path.join(__dirname, 'public', 'js', 'super-admin.js');
...
js = js.replace(/document\.getElementById\((['"`])([^'"`]+)\1\)\.textContent\s*=\s*([^;]+);/g, (match, quote, id, val) => {
    // Only animate stat identifiers or growth/notes identifiers
...
// Replace shortened assignment: s('X').textContent = Y
js = js.replace(/s\((['"`])([^'"`]+)\1\)\.textContent\s*=\s*([^;]+);/g, (match, quote, id, val) => {
    if (id.startsWith('stat') || id.startsWith('growth') || id.startsWith('notesStat') || id.startsWith('analytics') || id.startsWith('usage') || id.startsWith('org') || id.startsWith('sec') || id.startsWith('sys') || id.startsWith('m') || id.startsWith('rev')) {
        return `if(window.UIAnim){ UIAnim.animateValue(s('${id}'), 0, parseFloat(${val})||0); } else { s('${id}').textContent = ${val}; }`;
    }
    return match;
});
...
console.log('Successfully injected animateValue into super-admin.js');
